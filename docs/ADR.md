...
### ADR-009: 부분 성공을 정상 결과로 취급
**결정**: 자동 채우기에서 일부 필드만 채워져도 error가 아니라 partial 결과로 반환한다.

**이유**: 실제 지원 페이지는 사이트마다 구조가 다르고, 모든 필드를 채우는 것보다 잘못 채우지 않는 것이 중요하다. 사용자는 일부라도 정확히 채워지면 시간을 절약할 수 있다.

**트레이드오프**: 사용자는 남은 필드를 직접 채워야 한다. popup이 결과를 잘 설명하지 못하면 실패처럼 느껴질 수 있다.

**구현 규칙**:
- `filledCount > 0`이고 실패/스킵이 있으면 `partial`로 표시한다.
- `filledCount === 0`이고 매칭 후보가 없으면 `no-match`로 표시한다.
- 예외나 탭 접근 실패처럼 실행 자체가 불가능한 경우만 `error`로 표시한다.
- popup은 partial을 부정적인 오류처럼 보이게 하지 않는다.

### ADR-010: 프로필 입력은 점진적 완성을 허용
**결정**: MVP에서는 사용자가 모든 섹션을 완성하지 않아도 저장과 자동 채우기를 허용한다.

**이유**: 처음부터 학력, 자격사항, 프로젝트를 모두 입력하게 하면 사용자가 시작하기 전에 이탈할 수 있다. 이름, 이메일, 전화번호만 저장해도 반복 입력 문제의 일부를 바로 해결할 수 있다.

**트레이드오프**: 프로필이 부분적으로 비어 있으면 자동 채우기 결과도 제한된다.

**구현 규칙**:
- 필수 입력은 최소화한다.
- 빈 값은 저장 가능하지만 자동 채우기 plan에는 포함하지 않는다.
- options UI는 섹션별 완성도를 가볍게 보여줄 수 있지만, MVP 필수 기능은 아니다.
