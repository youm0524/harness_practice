...
const CREDENTIAL_CONTEXT_ALIASES = [
  "자격",
  "자격사항",
  "자격증",
  "면허",
...
const LANGUAGE_CONTEXT_ALIASES = [
  "어학",
  "외국어",
  "영어",
  "토익",
...
function formatPhoneValue(
  ...
  const maxLength = candidate.maxLength > 0 ? candidate.maxLength : 0;
  const compactHints = [candidate.name, candidate.id, candidate.autocomplete]
    .join(" ")
    .toLowerCase();

  if (
    /앞|first|prefix|tel1|phone1|mobile1|hp1/.test(text + " " + compactHints)
  ) {
    return first;
  }
...
function profileValues(profile: UserProfile): ProfileValue[] {
  const values: ProfileValue[] = [
    fieldValue(
      ...
      ["이름", "성명", "지원자명", "name", "full name", "applicant name"],
      90,
    ),
    dateFieldValue(
      "personal.birthDate",
  ...
  profile.educations.forEach((education, index) => {
    ...
    values.push(
      ...
      dateFieldValue(
        `${prefix}.startDate`,
        education.startDate,
        ["입학일", "입학년월", "입학", "시작일", "start date", "admission"],
        60,
        ["학력", "학교", "education"],
      ),
      dateFieldValue(
        `${prefix}.endDate`,
        education.endDate,
        ["졸업일", "졸업년월", "졸업", "종료일", "end date", "graduation"],
        60,
        ["학력", "학교", "education"],
      ),
      fieldValue(
  ...
  profile.credentials.forEach((credential, index) => {
    ...
    values.push(
      ...
      dateFieldValue(
        ...
        ["취득일", "취득년월", "발급일", "issued date", "acquired date"],
        60,
        CREDENTIAL_CONTEXT_ALIASES,
      ),
      fieldValue(
        `${prefix}.issuer`,
        credential.issuer,
        ...
        60,
        { contextAliases: CREDENTIAL_CONTEXT_ALIASES },
      ),
    );
  });

  profile.languageScores.forEach((languageScore, index) => {
    const prefix = `languageScores.${index}`;
    values.push(
      fieldValue(
        `${prefix}.language`,
        ...
        ["언어", "외국어", "language"],
        68,
        { contextAliases: LANGUAGE_CONTEXT_ALIASES },
      ),
      fieldValue(
        `${prefix}.testName`,
        languageScore.testName,
        ["시험명", "어학시험", "외국어시험", "test name", "exam"],
        72,
        { contextAliases: LANGUAGE_CONTEXT_ALIASES },
      ),
      fieldValue(
        `${prefix}.score`,
        languageScore.score,
        ["점수", "등급", "성적", "score", "grade", "level"],
        72,
        { contextAliases: LANGUAGE_CONTEXT_ALIASES },
      ),
      dateFieldValue(
        `${prefix}.acquiredDate`,
        languageScore.acquiredDate,
        ["취득일", "응시일", "발급일", "test date", "acquired date"],
        60,
        LANGUAGE_CONTEXT_ALIASES,
      ),
    );
  });
  ...
  profile.extracurricularProjects.forEach((project, index) => {
    ...
    values.push(
      fieldValue(
        ...
        80,
        { contextAliases: ACTIVITY_CONTEXT_ALIASES },
      ),
      fieldValue(
        `${prefix}.organization`,
        project.organization,
        ["활동기관", "소속기관", "주최기관", "기관명", "organization", "host"],
        82,
        { contextAliases: ACTIVITY_CONTEXT_ALIASES },
      ),
      dateFieldValue(
        `${prefix}.startDate`,
        project.startDate,
        ["시작일", "시작년월", "활동 시작", "기간 시작", "project start"],
        60,
      ...
      dateFieldValue(
        `${prefix}.endDate`,
        project.endDate,
        ["종료일", "종료년월", "활동 종료", "기간 종료", "project end"],
        60,
        ACTIVITY_CONTEXT_ALIASES,
      ),
      fieldValue(
      ...
      fieldValue(
        `${prefix}.techStack`,
        project.techStack,
        ["기술 스택", "사용 기술", "tech stack", "technologies"],
        70,
        { contextAliases: ACTIVITY_CONTEXT_ALIASES },
      ),
    );
  ...
  profile.workExperiences.forEach((experience, index) => {
    ...
    values.push(
      fieldValue(
        `${prefix}.company`,
        experience.company,
        ["회사명", "직장명", "근무처", "회사", "기관명", "company", "employer"],
        78,
        { contextAliases: WORK_CONTEXT_ALIASES },
      ),
      fieldValue(
        `${prefix}.position`,
        experience.position,
        ["직무", "직위", "직책", "담당업무", "position", "job title"],
        72,
        { contextAliases: WORK_CONTEXT_ALIASES },
      ),
      dateFieldValue(
        ...
        ["입사일", "근무 시작", "시작일", "기간 시작", "start date"],
        60,
        WORK_CONTEXT_ALIASES,
      ),
      dateFieldValue(
        `${prefix}.endDate`,
        experience.endDate,
        ["퇴사일", "근무 종료", "종료일", "기간 종료", "end date"],
        60,
        WORK_CONTEXT_ALIASES,
      ),
      fieldValue(
  ...
  profile.awards.forEach((award, index) => {
    ...
    values.push(
      fieldValue(
        `${prefix}.name`,
        award.name,
        ["수상명", "상훈명", "수상 항목", "상명", "award name"],
        72,
        { contextAliases: AWARD_CONTEXT_ALIASES },
      ),
      dateFieldValue(
        `${prefix}.awardDate`,
        award.awardDate,
        ["수상일", "수상년월", "award date"],
        60,
        AWARD_CONTEXT_ALIASES,
      ),
      fieldValue(
        `${prefix}.issuer`,
        award.issuer,
        ["수여기관", "발급기관", "주최기관", "기관", "issuer", "organization"],
        60,
        { contextAliases: AWARD_CONTEXT_ALIASES },
      ),
      fieldValue(
...
function scoreCandidate(text: string, profileValue: ProfileValue): number {
  ...
  if (
    isDateField(field) &&
    !/\d/.test(text) &&
    !/(일|월|년|date|day|month|year|입학|졸업|취득|수상|입사|퇴사|시작|종료)/.test(
      text,
    )
  ) {
    return 0;
