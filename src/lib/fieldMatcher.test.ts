...
describe("fieldMatcher", () => {
  ...
  it("matches Korean and English aliases", () => {
    ...
    expect(plan.matches.map((match) => [match.candidateKey, match.value])).toEqual([
      ["name", "홍길동"],
  ...
  it("uses section context to map generic textarea labels to the right experience domain", () => {
    ...
    const plan = buildAutofillPlan(
      [
        candidate({ key: "activity-desc", tagName: "textarea", type: "textarea", label: "내용", sectionText: "대내외활동 프로젝트 경험" }),
        candidate({ key: "work-desc", tagName: "textarea", type: "textarea", label: "내용", sectionText: "경력사항 근무경험" }),
        candidate({ key: "award-desc", tagName: "textarea", type: "textarea", label: "내용", sectionText: "수상 및 상훈" })
      ],
      profile
    );

    expect(plan.matches.map((match) => [match.candidateKey, match.profileField, match.value])).toEqual([
      ["activity-desc", "extracurricularProjects.0.description", "지원서 자동화 서비스를 구현했습니다."],
      ["work-desc", "workExperiences.0.description", "운영 도구를 개선했습니다."],
  ...
  it("treats related activity wording as the extracurricular project domain", () => {
    ...
    const plan = buildAutofillPlan(
      [candidate({ key: "activity-title", label: "활동명", sectionText: "교내외 활동경험" })],
      profile
    );
